...
<fieldset class="fieldset5">
    <legend><b><%=index_title%>&nbsp;[<a href='<%=Utility.getRoot() %>/memov3/list.jsp'>more...</a>]</b>
    </legend> 
   ...
   if (list.size() == 0) {
     ...
     for(int i=0; i< list.size(); i++){
       MemoVO vo = (MemoVO)list.get(i);
       int memono = vo.getMemono();
       String title = vo.getTitle();
       out.println("<img src='"+ Utility.getRoot() +"/memov3/images/bu3.gif'></img>");
       %>
       <!----------글 읽기----------->
       <%-- <a href='<%=Utility.getRoot() %>/memov3/read.jsp?memono=<%=memono %>'> --%>
       <%
        if (title.length() > 95){
