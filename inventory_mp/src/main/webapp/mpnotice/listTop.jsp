...
<fieldset class="fieldset5">
    <legend><b><%=index_title%>&nbsp;[<a href='<%=Utility.getRoot() %>/mpnotice/list_b.jsp'>more...</a>]</b>
    </legend> 
   ...
   if (list.size() == 0) {
     ...
     for(int i=0; i< list.size(); i++){
    	 MpnoticeDTO dto = (MpnoticeDTO)list.get(i);
       int noticeno = dto.getNoticeno();
       String title = Utility.convertChar(dto.getTitle());
       out.println("<img src='"+ Utility.getRoot() +"/mpnotice/images/bu3.gif'></img>");
       %>
       <!----------글 읽기----------->
       <a href='<%=Utility.getRoot() %>/mpnotice/read.jsp?noticeno=<%=noticeno %>'>
       <%
        if (title.length() > 95){
