...
<fieldset class="fieldset5">
    <legend><b><%=index_title%>&nbsp;[<a href='<%=Utility.getRoot() %>/clientbbs/list.jsp'>more...</a>]</b>
    </legend> 
   ...
   if (list.size() == 0) {
     ...
     for(int i=0; i< list.size(); i++){
       ClientbbsDTO dto = (ClientbbsDTO)list.get(i);
       int qnano = dto.getQnano();
       String title = Utility.convertChar(dto.getTitle());
       out.println("<img src='"+ Utility.getRoot() +"/clientbbs/images/bu3.gif'></img>");
       %>
       <!----------글 읽기----------->
       <a href='<%=Utility.getRoot() %>/clientbbs/read.jsp?qnano=<%=qnano %>'>
       <%
        if (title.length() > 95){
