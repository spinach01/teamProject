...
public class Paging { // 공통 클래스(common class) // css까지 자체 제공
  /**
   * 숫자형태의 페이징 1 페이지부터 시작
  ...
  public String paging(int totalRecord, int nowPage, int recordPerPage, String col, String word){
    ...
    for(int i=startPage; i<=endPage; i++){
      ...
      if (nowPage == i){ // 현재 페이지이면 강조 효과
        str.append("<span style='font-size: 1.2em; font-weight: bold;'>"+i+"</span>&nbsp;");  //20%확대
      }else{
  ...
  public String paging2(int totalRecord, int nowPage, int recordPerPage, String col, String word){
    ...
    int totalGrp = (int)(Math.ceil((double)totalPage/pagePerBlock));// 전체 그룹
    int nowGrp = (int)(Math.ceil((double)nowPage/pagePerBlock));    // 현재 그룹
    int startPage = ((nowGrp - 1) * pagePerBlock) + 1; // 특정 그룹의 페이지 목록 시작 // 10페이지 기준임..
    int endPage = (nowGrp * pagePerBlock);             // 특정 그룹의 페이지 목록 종료  
    ...
    int _nowPage = (nowGrp-1) * pagePerBlock; // 10개 이전 페이지로 이동
    if (nowGrp >= 2){
      str.append("<A href='./list.jsp?col="+col+"&word="+word+"&nowPage="+_nowPage+"'><span class='span_box_1'>이전</span></A>&nbsp;");
    }
    ...
    _nowPage = (nowGrp * pagePerBlock)+1; // 10개 다음 페이지로 이동
    if (nowGrp < totalGrp){
      str.append("<A href='./list.jsp?col="+col+"&word="+word+"&nowPage="+_nowPage+"'><span class='span_box_1'>다음</span></A>&nbsp;");
    }
    str.append("</DIV>");
...
//  public String paging4(int totalRecord, int nowPage, int recordPerPage, int mpgroupno, String mpname, String mpftype){
    ...
    int _nowPage = (nowGrp-1) * pagePerBlock; // 10개 이전 페이지로 이동
    if (nowGrp >= 2){
      str.append("<A href='./list.do?nowPage="+_nowPage+"&mpgroupno="+mpgroupno+"&colName="+colName+"&colFtype="+colFtype+"'><span class='span_box_1'>이전</span></A>&nbsp;");
    }
    ...
    _nowPage = (nowGrp * pagePerBlock)+1; // 10개 다음 페이지로 이동
    if (nowGrp < totalGrp){
      str.append("<A href='./list.do?nowPage="+_nowPage+"&mpgroupno="+mpgroupno+"&colName="+colName+"&colFtype="+colFtype+"'><span class='span_box_1'>다음</span></A>&nbsp;");
    }
    str.append("</DIV>");
